...
void ofApp::update() {
	...
	ofColor color;
	color.setHsb((ofGetFrameNum() * 2) % 255, 255, 230);
	this->color_list.push_back(color);

	for (int i = this->radius_list.size() - 1; i > -1; i--) {

		this->radius_list[i] += 1.5;
	...
	for (int i = 0; i < this->radius_list.size(); i++) {
		...
		this->setRingToMesh(this->face, this->frame, glm::vec3(), this->rotation_list[i], this->radius_list[i], this->radius_list[i] * 0.1, ofColor(this->color_list[i]), ofColor(255));
	}
}
...
void ofApp::draw() {
	...
	this->cam.begin();
	ofRotateY(ofGetFrameNum() * 2.66666666666666);
